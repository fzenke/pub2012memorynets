...
int main(int ac,char *av[]) {
	...
	PoissonGroup * poisson = new PoissonGroup( 2000, ext);
	...
	con_ee->load_patterns(infilename, chi, true);
	...
	if ( infilename != "" ) {
		...
		StimulusGroup * stimgroup = new StimulusGroup(1000,infilename,filename.str().c_str(),RANDOM,1);
		...
		StimulusGroup * stimgroup2 = new StimulusGroup(1000,infilename,filename.str().c_str(),SEQUENTIAL,1);
		stimgroup2->set_mean_on_period(1);
		stimgroup2->set_mean_off_period(30);
		stimgroup2->scale = 200;
		SparseConnection * con_stim2 = new SparseConnection(stimgroup2,neurons_i2,wei1,sparseness_e);
